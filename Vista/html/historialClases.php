...
<html>
    ...
    <body>
        ...
        if (pg_num_rows(pg_query($conn, "SELECT id_servicio, fecha, tipo_servicio, materia, descripcion, estado
            ...
            $arr = pg_fetch_all(pg_query($conn, "SELECT nombre, apellido, id_servicio, fecha, hora_inicio, hora_fin, tipo_servicio, materia, descripcion, estado
                                        FROM Servicio
                                        INNER JOIN Alumno
                                        ON Servicio.id_alumno = Alumno.id_alumno 
                                        INNER JOIN Materia
                                        ON Servicio.id_materia = Materia.id_materia WHERE Servicio.id_profesor=$id_profesor                                     
                                        ORDER BY fecha DESC"));
            ?>  <table id="Reservas" style="position: relative; top: 15%;"><tr><th>Id Servicio</th><th>Fecha</th><th>Hora</th><th>Tipo servicio</th><th>Alumno</th><th>Materia</th><th>Descripcion</th><th>Estado</th></tr><?php
        ...
        <div style="display: block; position: relative; margin-right: 50%; top: 20px;">
